...
"""
subnet_sweeper 'sweeps' subnets pinging all IPs in a subnet and return list
of hosts or IPs that respond to ping. This file also contains a generaton
function that RNG-spawns hosts within a subnet, for network_graph testing.
"""
...
def subnet_sweep(range_file="subnet_range.csv"):
    ...
    This function ingests a subnet file with ranges in the form of:
        ...
        Ex: 192.168.1.0,24 Will scan the entire 192.168.1.0/24 subnet
            from .1 to .255
...
def subnet_discovery(self_ip="127.0.0.1"):
    ...
    # Check to ensure octets are legal
    if (len(ip_octets) != 4
        or not all(ip_octet.isdigit() and 0 <= int(ip_octet) <= 255
        for ip_octet in ip_octets)):
        raise ValueError(f"Invalid IPv4 address: {ip_octets}")
    # End of IF
